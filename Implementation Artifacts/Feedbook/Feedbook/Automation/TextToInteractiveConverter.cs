...
{
    internal class TextToInteractiveConverter : IValueConverter
    {
            ...
            if (valueStr != null)
            {
                try
                {
                                   ...
                                   SplitAction(greaterWord, '<', (lessWord) =>
                                       {
                                           ...
                                           {
                                               var word = words[j];
                                               if (word != null && (word.StartsWith("http://") || word.StartsWith("https://")) && Uri.IsWellFormedUriString(word, UriKind.RelativeOrAbsolute))
                                               {
                                                   lineBuilder.Append("\"/>");
                    ...
                    catch (Exception ex)
                    {
                        ...
                        this.Log("Error occurred while loading google buzz feed after converting to XAML", ex);
                    }
                }
                catch (Exception ex)
                {
                    ex.Data["Feed"] = value;
                    this.Log("Error occurred while converting text of google buzz feed into XAML", ex);
                }
            }
